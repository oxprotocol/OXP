...
export async function GET(
  ...
  try {
    ...
    console.error("[versions GET] failed:", msg, stack);
    return NextResponse.json(
      {
        ok: false,
...
export async function POST(
  ...
  try {
    ...
    let publishVersion: (typeof import("@/lib/publish"))["publishVersion"];
    try {
      ({ publishVersion } = await import("@/lib/publish"));
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      const stack = err instanceof Error ? err.stack : undefined;
      console.error("[publish import] failed:", msg, stack);
      return NextResponse.json(
        {
          ok: false,
          error: "publish module load failed",
          message: msg,
          stack: stack?.split("\n").slice(0, 8),
        },
        { status: 500 },
    ...
    const msg = err instanceof Error ? err.message : String(err);
    const stack = err instanceof Error ? err.stack : undefined;
    console.error("[versions POST] failed:", msg, stack);
    return NextResponse.json(
      {
        ok: false,
        error: "internal error",
